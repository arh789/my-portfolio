...
These terms are passed into a clustering process using NetworkX, which calculates structural centrality to form distinct semantic zones. Underrepresented or low-significance terms are flagged using contrastive GPT audits and graph weight analysis. The result is a unified semantic field composed of three elements:
- Core terms (high centrality, strong relevance),
- Periphery terms (low centrality but contextually anchored), and
- Excluded terms (removed from the campaign field due to poor alignment or contextual risk).
...
- **Periphery**: Adjacent terms with lower cost-per-click and provisional relevance.
...
- Underperforming segments are removed or deprioritised
...
The final stage monitors campaign query data and drift in language patterns. If new candidate terms emerge - either via LLM suggestion or observed query echo - they are tested for alignment and added to the field if confirmed. Otherwise, they are excluded and logged.
...
- Exclusion logic reduces brand risk and filters low-value traffic at the structural level.
